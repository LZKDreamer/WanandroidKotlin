 ...
 * Function:
 */
class WxChildPresenter() : BasePresenter<WxChildContract.View>(),WxChildContract.Presenter{
    override fun requestWxArticle(pageNum: Int, id: Int) {
        RetrofitUtil.getService().getWxArticles(id,pageNum).enqueue(object :
            Callback<BaseBean<ArticlesPageBean<MutableList<ArticleBean>>>> {
                ...
                if (result == null){
                    ...
                    if (result.isSuccess()){
                        if (result.data.datas.isNullOrEmpty()){
                            ...
                            getView()?.requestWxArticleSuccess(result.data.pageCount,result.data.datas)
                        }
                    }else{
