...
class SquarePresenter() : BasePresenter<SquareContract.View>(),SquareContract.Presenter{
    ...
    override fun requestSquareList(page: Int) {
        RetrofitUtil.getService().getSquareList(page).enqueue(object : Callback<BaseBean<ArticlesPageBean<MutableList<ArticleBean>>>>{
                ...
                if (result == null){
                    ...
                    if (result.isSuccess()){
                        if (result.data.datas.isNullOrEmpty()){
                            ...
                            getView()?.requestSquareListSuccess(result.data.pageCount,result.data.datas)
                        }
                    }else{
